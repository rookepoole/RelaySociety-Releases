...
Relay Society beta updates are manual and user-confirmed. The v0.3.77 runtime does not silently download or execute a replacement.
...
The exact v0.3.71/schema-40 to v0.3.72/schema-41 adjacent migration and v0.3.72-to-v0.3.73 same-schema upgrade remain qualified. The exact v0.3.73/schema-41 to v0.3.75/schema-43 migration and v0.3.75/schema-43 to v0.3.76/schema-44 migration install the cultural schemas transactionally. The exact v0.3.76-to-v0.3.77 same-schema upgrade is also qualified on Windows Credential Manager and Linux Secret Service. These paths preserve canonical task, event, artifact, receipt, settlement, and culture state, keep platform custody unchanged, verify the prior receipt, and pass authenticated backup/restore plus post-restore diagnostics. This evidence does not replace the user's own backup.
